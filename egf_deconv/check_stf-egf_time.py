...
fout_p = 'output/eg_stf-egf_%s_time-p.pdf'%net_sta
fout_s = 'output/eg_stf-egf_%s_time-s.pdf'%net_sta
# signal process
samp_rate = 100
freq_band = [0.1,20]
p_win = [1,9]
...
for egf_idx in egf_idx_list:
  ...
  for ii in range(3):
    time_p, stf_p = deconv_iterative(tar_p[ii], egf_p[ii], samp_rate, dt_p, gauss_a, iter_max=iter_max, derr_min=derr_min, only_pos=only_pos)[0:2]
    time_s, stf_s = deconv_iterative(tar_s[ii], egf_s[ii], samp_rate, dt_s, gauss_a, iter_max=iter_max, derr_min=derr_min, only_pos=only_pos)[0:2]
...
for jj in range(3):
  ...
  for ii, stf_s in enumerate(stf_s_list):
    ...
    plt.plot(time, stf, color=colors[ii], lw=line_wid)
  plt.yticks(np.arange(len(egf_idx_list)), egf_idx_list)
  if jj==0: plot_label('Time (s)', 'EGF Index', '%s S-STF'%['E','N','Z'][jj])
