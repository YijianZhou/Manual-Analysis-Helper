...
fout_p = 'output/eg_stf-sta_%s_freq-p.pdf'%net_sta
fout_s = 'output/eg_stf-sta_%s_freq-s.pdf'%net_sta
# signal process
samp_rate = 100
freq_band = [0.1,20]
p_win = [1, 9]
s_win = [1, 15]
wl_list = [0.001, 0.01, 0.1] # water level
gauss_a = 10
dt_p, dt_s = 1, 2.
num_workers = 10
# fig config
...
for wl in wl_list:
  ...
  stf_p_list.append([])
  for ii in range(3):
    time_p, stf_p, tar_p_fit = deconv_waterlevel(tar_p[ii], egf_p[ii], samp_rate, dt_p, gauss_a, wl)[0:3]
    time_s, stf_s, tar_s_fit = deconv_waterlevel(tar_s[ii], egf_s[ii], samp_rate, dt_s, gauss_a, wl)[0:3]
    stf_p_list[-1].append([time_p, stf_p, tar_p_fit])
    stf_s_list[-1].append([time_s, stf_s, tar_s_fit])
...
# stf with different wl
for ii, wl in enumerate(wl_list):
  plt.subplot(3,2,2*ii+2)
  for jj in range(3):
    time_p, stf_p = stf_p_list[ii][jj][0:2]
    stf = stf_p / np.amax(stf_p) - 2*jj
    plt.plot(time_p, stf, color='tab:blue', alpha=alpha, lw=line_wid)
  plt.yticks([0,-2,-4],['E','N','Z'])
  xlabel = 'Shifted Time (s)' if ii==2 else None
  plot_label(xlabel, None, 'Water level = %s'%wl)
# plot stacked STF
plt.subplot(325)
stf_list = stf_p_list[1]
stf = [stf/np.amax(stf) for _,stf,_ in stf_list]
...
plt.legend(fontsize=fsize_label)
plot_label('Shifted Time (s)', None, 'P-wave STF (wl = %s)'%wl_list[1])
plt.tight_layout()
plt.savefig(fout_p)
...
plt.legend(fontsize=fsize_label)
plot_label('Shifted Time (s)', None, 'S-wave STF (wl = %s)'%wl_list[1])
plt.tight_layout()
plt.savefig(fout_s)
